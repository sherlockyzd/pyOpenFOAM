...
class Assemble:
    ...
    def cfdPreAssembleContinuityEquation(self,Region,*args):
        ...
        Sf = np.asarray(Region.mesh.faceSf)[0:theNumberOfInteriorFaces,:]
        # % Calculated info
        e = np.asarray(Region.mesh.faceCFn)[0:theNumberOfInteriorFaces,:]
        DUSf = np.asarray([DU0_f*Sf[:,1],DU1_f*Sf[:,2],DU2_f*Sf[:,2]]).T
        Region.fluid['DUSf'].phi[0:theNumberOfInteriorFaces,:]=DUSf
        magDUSf = np.asarray(mth.cfdMag(DUSf))
        ...
        for iBPatch, theBCInfo in Region.mesh.cfdBoundaryPatchesArray.items():    
            ...
            iBFaces=np.asarray(Region.mesh.cfdBoundaryPatchesArray[iBPatch]['iBFaces'])
            CF_b = np.asarray(Region.mesh.faceCF)[iBFaces]
            e = mth.cfdUnit(CF_b)
    ...
    def cfdAssembleDCoefficients(self,Region,iComponent): 
        ...
        Region.fluid[theDUField].phi[0:theNumberOfElements] = np.asarray(Region.mesh.elementVolumes/Region.coefficients.ac)[:,np.newaxis]
        Region.fluid[theDUTField].phi[0:theNumberOfElements]= np.asarray(Region.coefficients.ac_old/Region.coefficients.ac)[:,np.newaxis]
    ...
    # Get info and fields
        for iElement in range(Region.mesh.numberOfElements):
            ...
            for k in range(theNumberOfNeighbours):
                # if Region.coefficients.anb[iElement][k]>0:
                #     Region.coefficients.anb[iElement][k] = 0
    ...
    def cfdAssembleEquationTerms(self,Region,*args): 
        ...
        Region.fluid['rho'].cfdGetSubArrayForInterior(Region)
        Region.fluid['rho'].cfdGetPrevTimeStepSubArrayForInterior(Region)
        Region.fluid['mdot_f'].cfdGetSubArrayForInterior(Region)

        Region.fluid['rho'].cfdGetSubArrayForBoundary(Region)
        Region.fluid['rho'].cfdGetPrevTimeStepSubArrayForBoundary(Region)
        Region.fluid['mdot_f'].cfdGetSubArrayForBoundary(Region)

        # theScheme =Region.dictionaries.fvSchemes['divSchemes']['div(phi,'+theEquationName+')']
        if args:
            iComponent = args[0]
            self.iComponent=iComponent
            Region.fluid[theEquationName].cfdGetSubArrayForInterior(Region,iComponent)
            Region.fluid[theEquationName].cfdGetPrevTimeStepSubArrayForInterior(Region,iComponent)
            Region.fluid[theEquationName].cfdGetSubArrayForBoundary(Region,iComponent)
            Region.fluid[theEquationName].cfdGetPrevTimeStepSubArrayForBoundary(Region,iComponent)
            # if theScheme=='Gauss linear':
            Region.fluid[self.theEquationName].phiGrad.cfdGetGradientSubArrayForInterior(Region,iComponent)
        else:
            self.iComponent=-1
            Region.fluid[self.theEquationName].cfdGetSubArrayForInterior(Region)
            Region.fluid[self.theEquationName].cfdGetPrevTimeStepSubArrayForInterior(Region)
            Region.fluid[self.theEquationName].cfdGetSubArrayForBoundary(Region)
            Region.fluid[self.theEquationName].cfdGetPrevTimeStepSubArrayForBoundary(Region)
            # if theScheme=='Gauss linear':
    ...
    def cfdAssembleContinuityEquationTerms(self,Region,*args): 
        ...
        Assembles the equation's terms
        """
        # print('Inside ContinuityEquationTerms')
        # self.cfdZeroElementFLUXCoefficients(Region)
        theEquationName=self.theEquationName
        Region.fluid['rho'].cfdGetSubArrayForInterior(Region)
        Region.fluid['rho'].cfdGetPrevTimeStepSubArrayForInterior(Region)
    ...
    def cfdAssembleMassDivergenceTermInterior(self,Region,*args):
        ...
        Sf = np.asarray(Region.mesh.faceSf)[0:theNumberOfInteriorFaces,:]
        cfdMagCF = np.asarray(Region.mesh.faceDist)[0:theNumberOfInteriorFaces]
    ...
    def assembleFirstOrderEulerTransientTerm(self,Region,*args):
        ...
        2. **获取元素体积**：
        - `self.volumes = np.asarray(Region.mesh.elementVolumes).reshape(-1,1)` 将区域网格的元素体积转换为NumPy数组，并重塑为二维列向量。
        ...
        # Region.fluid[self.theEquationName].cfdGetPrevTimeStepSubArrayForInterior(Region)
        ## phi for interior
        # self.phi=Region.fluid[self.theEquationName].phiInteriorSubArray
        ## phi for interior in previous time step
        # self.phi_old=Region.fluid[self.theEquationName].phi_oldInteriorSubArray
        # if self.theEquationName=='U':
            # Region.fluid['rho'].cfdGetSubArrayForInterior(Region)
            # Region.fluid['rho'].cfdGetPrevTimeStepSubArrayForInterior(Region)
            ## rho for interior
        # rho=Region.fluid['rho'].phiInteriorSubArray
        ## rho for interior in previous time step
        # rho_old=Region.fluid['rho'].phi_oldInteriorSubArray
        ## time step
        ...
        # self.volumes = np.asarray(Region.mesh.elementVolumes).reshape(-1,1)
        volumes = np.asarray(Region.mesh.elementVolumes)[:,np.newaxis]
        deltaT = Region.dictionaries.controlDict['deltaT']
        local_FluxC = np.squeeze(volumes*Region.fluid['rho'].phiInteriorSubArray)/deltaT 
        local_FluxC_old = -np.squeeze(volumes*Region.fluid['rho'].phi_oldInteriorSubArray)/deltaT
        ...
        # local_FluxT = np.squeeze(Region.fluxes.FluxC[:, np.newaxis]*self.phi+Region.fluxes.FluxC_old[:, np.newaxis]*self.phi_old)
        # if args:
        #     iComponent=args[0]
        #     Region.fluxes.FluxT= np.squeeze(Region.fluxes.FluxC*Region.fluid[self.theEquationName].phiInteriorSubArray+Region.fluxes.FluxC_old*Region.fluid[self.theEquationName].phi_oldInteriorSubArray)
        # else:
        Region.fluxes.FluxT= Region.fluxes.FluxC*np.squeeze(Region.fluid[self.theEquationName].phiInteriorSubArray)+Region.fluxes.FluxC_old*np.squeeze(Region.fluid[self.theEquationName].phi_oldInteriorSubArray)
    ...
    def processDCSOUScheme(self,Region):
        # % Get mesh info
        theElementCentroids = np.asarray(Region.mesh.elementCentroids)
        theFaceCentroids = np.asarray(Region.mesh.faceCentroids)
        numberOfInteriorFaces = Region.mesh.numberOfInteriorFaces

        # % Get fields
        theEquationName=self.theEquationName
        phi=Region.fluid[theEquationName].phiInteriorSubArray
        gradPhi = Region.fluid[theEquationName].phiGrad.phiGradInter
        mdot_f = Region.fluid['mdot_f'].phiInteriorSubArray
        # cfdGetSubArrayForInterior('mdot_f')
        pos = np.zeros(len(mdot_f),dtype=int)
        pos[np.squeeze(mdot_f) > 0] = 1
        iFaces = list(range(numberOfInteriorFaces))
        owners_f = np.asarray(Region.mesh.owners)[iFaces]
        neighbours_f = np.asarray(Region.mesh.neighbours)[iFaces]
    ...
    def cfdAssembleConvectionTermSpecifiedValue(self,Region,iBPatch,*args):
        ...
        Ui_C =Region.fluid[theEquationName].phiInteriorSubArray
        Ui_b =Region.fluid[theEquationName].phiBoundarySubArray
        mdot_b = Region.fluid['mdot_f'].phiBoundarySubArray
        ...
        Region.fluxes.FluxFf[iBFaces]  = local_FluxFb
        # Region.fluxes.FluxVf[iBFaces]  = local_FluxVb
        Region.fluxes.FluxTf[iBFaces]  = local_FluxCb*Ui_C[owners_b] + local_FluxFb*Ui_b[iBElements] #+ local_FluxVb
    ...
    def cfdAssembleConvectionTermZeroGradient(self,Region,iBPatch,*args):
        ...
        Region.fluxes.FluxFf[iBFaces]  = local_FluxFb
        # Region.fluxes.FluxVf[iBFaces]  = local_FluxVb
        Region.fluxes.FluxTf[iBFaces]  = local_FluxCb*Ui_C[owners_b] 
    ...
    def cfdAssembleConvectionTermInterior(self,Region,*args):
        ...
        # Region.fluid['mdot_f'].cfdGetSubArrayForInterior(Region)
        mdot_f=np.squeeze(Region.fluid['mdot_f'].phiInteriorSubArray)
    ...
    def cfdAssembleMomentumDivergenceCorrectionTerm(self,Region,*args):
        ...
        Ui=np.squeeze(Region.fluid[self.theEquationName].phiInteriorSubArray)
    ...
    def cfdAssembleDiffusionTermInterior(self,Region,*args):
        ...
        # CF = Region.mesh.faceCF
        # Ef = Region.mesh.faceEf
        # Tf = Region.mesh.faceCF
        phi=Region.fluid[theEquationName].phiInteriorSubArray
        ...
        if theEquationName=='U':
            ...
            if Region.cfdIsCompressible:
                local_FluxVf += 2.0/3.0*np.squeeze(gamma_f[0:numberOfInteriorFaces])*(Region.fluid[theEquationName].phiGrad.phiGrad_Trace[0:numberOfInteriorFaces]*np.asarray(Region.mesh.interiorFaceSf)[:,self.iComponent])
    ...
    def cfdAssembleStressTermWallNoslipBC(self,Region,iBPatch,*args):
        ...
        # v_b = U_b[:,1]
        # w_b = U_b[:,2]
        n = mth.cfdUnit(np.asarray(Region.mesh.faceSf)[iBFaces])
        ...
        # % Local fluxes
        if iComponent==0:
            local_FluxCb =  mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(1-nx2)
            local_FluxVb = -mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(v_C*ny*nx+w_C*nz*nx)
        elif iComponent==1:
            local_FluxCb =  mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(1-ny2)
            local_FluxVb = -mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(u_C*nx*ny+w_C*nz*ny)
        elif iComponent==2:
            local_FluxCb =  mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(1-nz2)
            local_FluxVb = -mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(u_C*nx*nz+v_C*ny*nz)
    ...
    def cfdAssembleStressTermWallFixedValueBC(self,Region,iBPatch,*args):
        ...
        if iComponent != -1:
            ...
            if iComponent==0:
                local_FluxCb =  mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(1-nx2)
                local_FluxVb = -mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(u_b*(1-nx2)+(v_C-v_b)*ny*nx+(w_C-w_b)*nz*nx)
            elif iComponent==1:
                local_FluxCb =  mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(1-ny2)
                local_FluxVb = -mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*((u_C-u_b)*nx*ny+v_b*(1-ny2)+(w_C-w_b)*nz*ny)
            elif iComponent==2:
                local_FluxCb =  mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(1-nz2)
                local_FluxVb = -mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*((u_C-u_b)*nx*nz+(v_C-v_b)*ny*nz+w_b*(1-nz2))
            local_FluxFb =np.zeros(Region.mesh.cfdBoundaryPatchesArray[iBPatch]['numberOfBFaces'])
        else:
            local_FluxCb=mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]
            local_FluxFb=-mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]
    ...
    def cfdAssembleStressTermSymmetry(self,Region,iBPatch,*args):
        ...
        # % Local fluxes
        if iComponent==0:
            local_FluxCb =  2*mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*nx2
            local_FluxVb =  2*mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(v_C*ny*nx+w_C*nz*nx)
        elif iComponent==1:
            local_FluxCb =  2*mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*ny2
            local_FluxVb =  2*mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(u_C*nx*ny+w_C*nz*ny)
        elif iComponent==2:
            local_FluxCb =  2*mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*nz2
            local_FluxVb =  2*mu_b*np.asarray(Region.mesh.geoDiff_f)[iBFaces]*(u_C*nx*nz+v_C*ny*nz)
    ...
    def cfdAssembleStressTermSpecifiedValue(self,Region,iBPatch):
        ...
        # % Get fields
        # Region.fluid[theEquationName].cfdGetSubArrayForInterior(Region)
        phi_C =Region.fluid[theEquationName].phiInteriorSubArray[owners_b]
        phi_b =Region.fluid[theEquationName].phiBoundarySubArray[iBElements]
    ...
    def cfdAssembleStressTermZeroGradientBC(self,Region,iBPatch):
        ...
        # # % Get fields
        # # Region.fluid[theEquationName].cfdGetSubArrayForInterior(Region)
        # U_C =Region.fluid[theEquationName].phiInteriorSubArray[owners_b]
        # U_b =Region.fluid[theEquationName].phiBoundarySubArray[iBElements]

        # Region.fluxes.FluxCf[iBFaces] = local_FluxCb
        # Region.fluxes.FluxFf[iBFaces] = local_FluxFb
        # Region.fluxes.FluxVf[iBFaces] = local_FluxVb
        # Region.fluxes.FluxTf[iBFaces] = local_FluxCb*U_C+ local_FluxFb*U_b + local_FluxVb
    ...
    def cfdAssembleMomentumBuoyancyTerm(self,Region,*args):
        ...
        # % Get fields
        rho = np.squeeze(Region.fluid['rho'].phiInteriorSubArray)
        # % Get gravity
    ...
    def cfdAssemblePressureGradientTerm(self,Region,*args):
        # % Get info and fields
        volumes = np.squeeze(Region.mesh.elementVolumes)
        p_grad = np.squeeze(Region.fluid['p'].phiGrad.phiGrad[0:Region.mesh.numberOfElements,self.iComponent])
        # % Update global fluxes
